...
class CHAIR(object):
    ...
    def caption_to_words(self, caption):
        ...
        #    double_raw = ' '.join(raw_words[i:i+2])
        #    if double_word in self.double_word_dict: 
        #        double_words.append(self.double_word_dict[double_word])
        #        raw_words_aligned.append(double_raw)
        #        i += 2
        #    else:
        #        double_words.append(words[i])
        #        raw_words_aligned.append(raw_words[i])
        #        i += 1
        # words = double_words
        ...
        # remove 'seat' if 'toilet' is also present
        if ('toilet' in words) & ('seat' in words): words = [word for word in words if word != 'seat']

        # keep only MSCOCO objects
        idxs = [idxs[idx] for idx, word in enumerate(words) \
                if word in set(self.mscoco_objects)]
        og_words = []
        for i in range(len(words)):
            if words[i] in set(self.mscoco_objects):
                og_words.append(raw_words_aligned[i])
        words = [word for word in words if word in set(self.mscoco_objects)]
        ...
        return og_words, node_words, idxs, raw_words_aligned
    ...
    def compute_chair(self, cap_file):
        ...
        for i, cap_eval in enumerate(caps):
            ...
            # CHAIRi
            for k, obj in enumerate(node_words):
                if not obj:
                    continue
                coco_word_count += 1
                if obj not in gt_objects:
                    hallucinated_word_count += 1
                    cap_dict['mscoco_hallucinated_words'].append(obj)
                else:
                    cap_dict['mscoco_non_hallucinated_words'].append(obj)

            cap_dict['metrics']['CHAIRi'] = (
                hallucinated_word_count / coco_word_count if coco_word_count else 0.0
            )
            chair_i.append(cap_dict['metrics']['CHAIRi'])

            # Coverage
            gt_set = set(gt_objects)
            gen_set = set(w for w in node_words if w)
            covered_word_count = len(gt_set & gen_set)
            cap_dict['metrics']['coverage'] = (covered_word_count / len(gt_set)) if gt_set else 0.0
            coverage.append(cap_dict['metrics']['coverage'])

            #  CHAIRs
            mergedtext = ' '.join(raw_words)
            sentences = [s.strip() for s in mergedtext.replace('!', '.').replace('?', '.').split('.') if s.strip()]
            ...
            for sent in sentences:
                ...
                for i in range(len(node_words)):
                    if not node_words[i]:
                        continue
                    if node_words[i] not in gt_objects and words[i] in sent.split():
                        has_hallu = True
                        break
                if has_hallu:
                    num_hallucinated_caps += 1

            cap_dict['metrics']['CHAIRs'] = (num_hallucinated_caps / len(sentences)) if sentences else 0.0
            chair_s.append(cap_dict['metrics']['CHAIRs'])
 
        chair_s_fin = sum(chair_s)/len(chair_s) if len(chair_s) > 0 else 0.
        chair_i_fin = sum(chair_i)/len(chair_i) if len(chair_i) > 0 else 0.
        coverage_fin = sum(coverage)/len(coverage) if len(coverage) > 0 else 0.
    
        output['overall_metrics'] = {
                                    'coverage': coverage_fin,
                                    'CHAIRs': chair_s_fin,
                                    'CHAIRi': chair_i_fin
                                    }
...
def load_generated_captions(cap_file):
    #Read in captions
    #Old function, for dedicated files        
    ...
    for line in all_caps:
        cap = json.loads(line.strip())
        caps.append({'image_id':cap['image_id'], 'caption': cap['description']})
        imids.append(cap['image_id'])
    imids = set(imids)
...
def save_hallucinated_words(cap_file, cap_dict): 
    ...
    with open('/home/user/khoihm/CHAIR_Paper/hallucinated_words_%s' %tag, 'w') as f:
...
def print_metrics(hallucination_cap_dict, quiet=False):
    ...
    metric_string = "%0.01f\t%0.01f\t%0.01f" %(
                                                  sentence_metrics['CHAIRs']*100,
                                                  sentence_metrics['CHAIRi']*100,
                                                  sentence_metrics['coverage']*100)

    if not quiet:
        print("CHAIRs\tCHAIRi\tCover")
        print(metric_string)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cap_file", type=str, default='/home/user/khoihm/PHASE2_postgen_agla_1_fin.jsonl')
    parser.add_argument("--annotation_path", type=str, default='/home/user/khoihm/annotations')
